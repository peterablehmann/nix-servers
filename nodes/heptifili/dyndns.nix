...
}:
{
  ...
  systemd.services."dyndns" = {
    script = ''
      ...
      # Get record ids
      recordid_ipv4=$(${pkgs.curl}/bin/curl -s "https://dns.hetzner.com/api/v1/records?zone_id=$zone_id" -H "Auth-API-Token: $(cat ${config.sops.secrets."dyndns/hetzner_api_key".path})" | ${pkgs.jq}/bin/jq '.records.[] | select(.name=="ip.heptifili") | select(.type=="A") | .id' | tr -d '"')
      recordid_ipv6=$(${pkgs.curl}/bin/curl -s "https://dns.hetzner.com/api/v1/records?zone_id=$zone_id" -H "Auth-API-Token: $(cat ${config.sops.secrets."dyndns/hetzner_api_key".path})" | ${pkgs.jq}/bin/jq '.records.[] | select(.name=="ip.heptifili") | select(.type=="AAAA") | .id' | tr -d '"')
      ...
      then
        ...
        ${pkgs.curl}/bin/curl -s -X "PUT" "https://dns.hetzner.com/api/v1/records/$recordid_ipv4" -H 'Content-Type: application/json' -H "Auth-API-Token: $(cat ${config.sops.secrets."dyndns/hetzner_api_key".path})" -d $'{"value": "'$current_ipv4'", "ttl": 60, "type": "A", "name": "'ip.heptifili'", "zone_id": "'$zone_id'"}'
      fi
